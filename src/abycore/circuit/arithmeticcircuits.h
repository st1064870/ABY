...
public:
    ...
    uint32_t PutFPGate(uint32_t left, uint32_t right);

    uint32_t PutINGate(e_role src);

    template<class T>
    uint32_t PutINGate(T val, e_role role) {
        uint32_t gateid = PutINGate(role);
        if (role == m_eMyRole) {
            GATE *gate = &(m_vGates[gateid]);
            gate->gs.ishare.inval = (UGATE_T *) calloc(ceil_divide(1 * m_nShareBitLen, sizeof(UGATE_T) * 8),
                                                       sizeof(UGATE_T));
    ...
    template<class T>
    uint32_t PutSIMDINGate(uint32_t nvals, T val, e_role role) {
        uint32_t gateid = PutSIMDINGate(nvals, role);
        if (role == m_eMyRole) {
            GATE *gate = &(m_vGates[gateid]);
            gate->gs.ishare.inval = (UGATE_T *) calloc(ceil_divide(nvals * m_nShareBitLen, GATE_T_BITS),
                                                       sizeof(UGATE_T));
    ...
    template<class T>
    uint32_t PutSharedINGate(T val) {
    ...
    template<class T>
    uint32_t PutSharedSIMDINGate(uint32_t nvals, T val) {
    ...
    share *PutFPGate(share *ina, share *inb);
...
private:
    ...
    template<class T>
    share *InternalPutSharedINGate(uint32_t nvals, T val, [[maybe_unused]] uint32_t bitlen) {
        share *shr = new arithshare(this);
    ...
    template<class T>
    share *InternalPutINGate(uint32_t nvals, T *val, uint32_t bitlen, e_role role) {
        assert(bitlen <= m_nShareBitLen);
        share *shr = new arithshare(this);
        uint32_t gateid = PutSIMDINGate(nvals, role);
        assert((sizeof(UGATE_T) / sizeof(T)) > 0);
    ...
    template<class T>
    share *InternalPutINGate(uint32_t nvals, T val, [[maybe_unused]] uint32_t bitlen, e_role role) {
        share *shr = new arithshare(this);
