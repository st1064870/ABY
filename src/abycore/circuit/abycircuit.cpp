...
uint32_t
ABYCircuit::PutNonLinearVectorGate(e_gatetype type, uint32_t choiceinput, uint32_t vectorinput, uint32_t rounds) {
...
uint32_t ABYCircuit::PutStructurizedCombinerGate(std::vector<uint32_t> input, uint32_t pos_start, uint32_t pos_incr,
                                                 uint32_t nvals) {
...
uint32_t
ABYCircuit::PutCallbackGate(std::vector<uint32_t> in, uint32_t rounds, void (*callback)(GATE *, void *), void *infos,
                            uint32_t nvals) {
...
void
ABYCircuit::ExportCircuitInBristolFormat(std::vector<uint32_t> ingates_client, std::vector<uint32_t> ingates_server,
    ...
    outfile << total_bristol_gates - ingates_client.size() - ingates_server.size() << " " << total_bristol_gates
            << std::endl;
    //Write the number of client / server inputs and output gates
    outfile << ingates_client.size() << " " << ingates_server.size() << "   " << outgates.size() << std::endl
            << std::endl;
    ...
    for (uint32_t i = 0; i < outgates.size(); i++) {
        ...
        for (uint32_t j = 0; j < ingates_client.size() && !skip; j++) {
            if (outgate_map[i] == ingates_client[j]) {
                skip = true;
                outfile << "2 1 " << ingates_client[j] << " " << zerogate << " "
                        << total_bristol_gates - outgate_map.size() + i << " XOR" << std::endl;
            }
        }
        for (uint32_t j = 0; j < ingates_server.size() && !skip; j++) {
            if (outgate_map[i] == ingates_server[j]) {
                skip = true;
                outfile << "2 1 " << ingates_server[j] << " " << zerogate << " "
                        << total_bristol_gates - outgate_map.size() + i << " XOR" << std::endl;
            }
        }
    }
...
void ABYCircuit::ExportGateInBristolFormat(uint32_t gateid, uint32_t &next_gate_id, std::vector<int> &gate_id_map,
    ...
    if (m_vGates[gateid].type == G_IN) {
        ...
        //enter gate into map
        if (constant_map[m_vGates[gateid].ingates.inputs.twin.left] +
            constant_map[m_vGates[gateid].ingates.inputs.twin.right] != -2) {
            CheckAndPropagateConstant(gateid, next_gate_id, gate_id_map, constant_map, outfile);
        } else {
            outfile << "2 1 " << gate_id_map[m_vGates[gateid].ingates.inputs.twin.left] << " "
                    << gate_id_map[m_vGates[gateid].ingates.inputs.twin.right] << " " << next_gate_id << " XOR"
                    << std::endl;
        ...
        if (constant_map[m_vGates[gateid].ingates.inputs.twin.left] +
            ...
            outfile << "2 1 " << gate_id_map[m_vGates[gateid].ingates.inputs.twin.left] << " "
                    << gate_id_map[m_vGates[gateid].ingates.inputs.twin.right] << " " << next_gate_id << " AND"
                    << std::endl;
            gate_id_map[gateid] = next_gate_id++;
        }
        ...
        if (constant_map[m_vGates[gateid].ingates.inputs.parent] != -1) {
            ...
            outfile << "1 1 " << gate_id_map[m_vGates[gateid].ingates.inputs.parent] << " " << next_gate_id << " INV"
                    << std::endl;
            gate_id_map[gateid] = next_gate_id++;
        }
        ...
        std::cerr << "Gate type not available in Bristol format: " << get_gate_type_name(m_vGates[gateid].type)
...
void ABYCircuit::CheckAndPropagateConstant(uint32_t gateid, uint32_t &next_gate_id, std::vector<int> &gate_id_map,
    ...
    if (constant_map[m_vGates[gateid].ingates.inputs.twin.left] == 0 &&
        constant_map[m_vGates[gateid].ingates.inputs.twin.right] == 0) {
    ...
    if (constant_map[m_vGates[gateid].ingates.inputs.twin.left] +
        constant_map[m_vGates[gateid].ingates.inputs.twin.right] == 1) {
        if (m_vGates[gateid].type == G_LIN) {
            constant_map[gateid] = 1;
        } else if (m_vGates[gateid].type == G_NON_LIN) {
            constant_map[gateid] = 0;
    ...
    if (constant_map[m_vGates[gateid].ingates.inputs.twin.left] +
        ...
        if (m_vGates[gateid].type == G_LIN) {
            if (constant_map[m_vGates[gateid].ingates.inputs.twin.left] == -1) {
                outfile << "1 1 " << gate_id_map[m_vGates[gateid].ingates.inputs.twin.left] << " " << next_gate_id
                        << " INV" << std::endl;
                gate_id_map[gateid] = next_gate_id++;
            } else {
                outfile << "1 1 " << gate_id_map[m_vGates[gateid].ingates.inputs.twin.right] << " " << next_gate_id
                        << " INV" << std::endl;
                gate_id_map[gateid] = next_gate_id++;
            }
        } else if (m_vGates[gateid].type == G_NON_LIN) {
